...
class GStepper2 : public Stepper<_DRV, _TYPE> {
   public:
    // ========================= КОНСТРУКТОР ==========================
    GStepper2(int steps, uint8_t pin1 = 255, uint8_t pin2 = 255, uint8_t pin3 = 255, uint8_t pin4 = 255, uint8_t pin5 = 255) : Stepper<_DRV, _TYPE>(pin1, pin2, pin3, pin4, pin5) {
        stepsRev = steps;
        setMaxSpeed(100);
        setAcceleration(200);
    }
    ...
    void setTargetDeg(double ntar, GS_posType type = ABSOLUTE) {
        setTarget(ntar * stepsRev / 360.0, type);
    }
